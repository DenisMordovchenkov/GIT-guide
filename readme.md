...
![fast-forward](https://pictures.s3.yandex.net/resources/M4_T2_01_1689342594.png)

Ветка `add-docs` «обгоняет» ветку `main` на два коммита: 
`N1` и `N2` (в нашем примере с кодом они называются `New docs 1` и `New docs 2`).
...
![merge result](https://pictures.s3.yandex.net/resources/M4_T2_02_1689342662.png)
...
![merge result](https://pictures.s3.yandex.net/resources/M4_T2_03_1689342784.png)
...
![non-fast-forward](https://pictures.s3.yandex.net/resources/M4_T2_01202_1689343530.png)

Теперь просто «положить» все коммиты из `add-docs` в `main` не получится. Например, коммит 5 *C5* из `main` может конфликтовать с каким-нибудь коммитом из ветки `add-docs` (*N1* или с *N2* на схеме ниже мы выбрали *N1* в качестве примера). То есть содержать изменения в тех же файлах (и в тех же строках), что и коммит 5 *C5*.

![conflict](https://pictures.s3.yandex.net/resources/M4_T2_02203_1689343586.png)
...
При слиянии *non-fast-forward* веток `Git` создаёт коммит слияния.
...
![merge-commit](https://pictures.s3.yandex.net/resources/M4_T2_03202_1689343628.png)
...
Одна из самых популярных `Git`-команд — это `git push` (англ. «толкнуть», «протолкнуть»). Она тесно связана с состоянием *fast-forward*. Посмотрим, что это за связь и откуда она берётся.

`git push: «толкает» коммиты из локальной ветки в её удалённую копию.
...
> ⚠️ Обычно локальная и удалённая ветки называются одинаково. Чтобы их различать, будем называть локальные ветки просто именем (например, `main`), а удалённые — с суффиксом `@origin` (например, `main@origin`).
...
В `Git` можно решить проблему «разошедшихся» веток с помощью операции `rebase` (англ. «перебазирование»). Эта операция позволяет изменить точку (коммит), от которой отделилась ветка. Результат перебазирования ветки `main` c коммита `C2` на коммит `D` можно увидеть на схеме.
